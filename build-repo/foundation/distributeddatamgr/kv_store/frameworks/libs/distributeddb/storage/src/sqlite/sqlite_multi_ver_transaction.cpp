    ...
    bool &isNeedSave, Value &origVal) const
{
    ...
    if (errCode == SQLiteUtils::MapSQLiteErrno(SQLITE_DONE)) {
        ...
        LOGD("Timestamp :%" PRIu64 " vs %" PRIu64 ", %" PRIu64 " vs %" PRIu64 ", readVersion:%" PRIu64 ", version:"
            "%" PRIu64 ", %d", readOriTime, oriTimestamp, readTime, timestamp, readVersion, version_, isNeedSave);
